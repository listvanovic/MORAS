class Parser:
    ...
    def parseFile(self, filename):
        ...
        #kupim sve labele u fileu
        try:
            self._readLabels()
        except:
            Parser._error("File", -1, "Could not parse labels.")
            return
    ...
    def _readLabels(self):
        for (line, n) in self._ilines:
            l = line.strip() #remove any trailing and leading whitespace
            words = l.split()
            if len(words) > 1 and words[0] == 'label':
                self._labels.append(words[1])
    ...
    def _goto(self, lab, n):
        if lab in self._labels:
            ...
            self._flag = False
            Parser._error("Goto", n, f"Invalid label({lab},{self._name}.vm)")
            return ""
    ...
    def _ifgoto(self, lab, n):
        if lab in self._labels:
            ...
            self._flag = False
            Parser._error("If-Goto", n, f"Invalid label({lab},{self._name}.vm)")
            return ""
    ...
    def _push(self, src, loc, n):
        try:
            ...
            if(x < 0):
                self._flag = False
                Parser._error("Push", n, f"The argument has to be a non negative whole number({loc}).")
                return ""
        except:
            self._flag = False
            Parser._error("Push", n, f"The argument has to be a non negative whole number({loc}).")
    ...
    def _pop(self, dst, loc, n):
        try:
            ...
            if(x < 0):
                self._flag = False
                Parser._error("Pop", n, f"The argument has to be a non negative whole number({loc}).")
                return ""
        except:
            self._flag = False
            Parser._error("Pop", n, f"The argument has to be a non negative whole number({loc}).")
...
def main():
    ...
    P.parseFile("bsMain")
    P.parseFile("divmod")
    P.parseFile("BinarySearch")
    P.writeFile("../zad2/out/bs")
